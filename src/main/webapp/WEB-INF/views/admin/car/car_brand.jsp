...
<script src="resources/js/admin/car/admin_car.js"></script>
			...
			<table id="admin_car_content">
				<tr>
					...
					<td class="admin_car_content_title admin_brand_each"
						style="border-right: 1px solid white;">등록차종수</td>
					<td class="admin_car_content_title admin_brand_order"
						style="border-right: 1px solid white;">표시순서</td>
	...
	<%--이거 왜있는거임?<c:forEach var="carcount" items="${carcount}" varStatus="status">
				</c:forEach> --%>
				<c:forEach items="${carbrands}" var="cb" varStatus="status">
					<tr id="admin_cars_content">
						...
						<td class="admin_car_table_td">
							<button type="button" class="updatecarbrandbutton"
								onclick="updatecb1('${cb.cb_name}','${cb.cb_order }')">수정</button>
			...
			<div>
				<form action="update.brand.do" name="updateform"
					...
					<table border="1" class="">
						...
						<tr>
							<td style="background-color: #3399ff">표시순서</td>
							<td><input name="cb_order" id="cb_order_u"
								style="width: 180px; height: 30px;"></td>
						</tr>
						<tr>
							<td colspan=2>
								<div class="carbrandupdateokbutton">
